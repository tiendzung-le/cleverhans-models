...
def main(_):
    ...
    with tf.Graph().as_default() as g:
        ...
        for _ in range(num_iter):
            # Resnet
            with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
                logits1, end_points1 = inception_resnet_v2.inception_resnet_v2(
                    x_adv, num_classes=num_classes, is_training=False, reuse=True)

            cross_entropy1 = tf.losses.softmax_cross_entropy(one_hot_target_class,
                                                            logits1,
                                                            label_smoothing=0.1,
                                                            weights=1.0)
            cross_entropy1 += tf.losses.softmax_cross_entropy(one_hot_target_class,
            ...
            # Inception
            with slim.arg_scope(inception.inception_v3_arg_scope()):
                logits2, end_points2 = inception.inception_v3(
                    x_adv, num_classes=num_classes, is_training=False, reuse=True)

            cross_entropy2 = tf.losses.softmax_cross_entropy(one_hot_target_class,
                                                            logits2,
                                                            label_smoothing=0.1,
                                                            weights=1.0)
            cross_entropy2 += tf.losses.softmax_cross_entropy(one_hot_target_class,
            ...
            # Combine: reduce the weight of inception
            x_next = x_adv - alpha * tf.sign(tf.gradients(cross_entropy1 + 0.7*cross_entropy2, x_adv)[0])
